...
export default function ChatList({ vibehiveUser }) {
  return (
    <div className="left min-h-screen h-max   ">
      <h1 className="text-5xl text-center tracking-wide  font-cookie  px-2   font-bold mb-5">
        Messenger
      </h1>
      <ul className="border border-[#616161] p-2 rounded-sm">
        {vibehiveUser.map((user, index) => (
          <Link to={`/messenger/${user?.id}`}>
            <li
              className="flex cursor-pointer items-center px-3 gap-2 hover:bg-[#363636] mx-2 p-1 rounded-sm py-2 transition-colors"
              key={index}
            >
